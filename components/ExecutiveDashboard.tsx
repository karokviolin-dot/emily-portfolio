...
function priorityScore(task: any) {  const base = task.priority === "high" ? 3 : task.priority === "medium" ? 2 : 1;
...
export default function ExecutiveOpsDashboard() {
  ...
  function dismissSuggestion(id, onAction) {
    onAction?.();
...
function Overview({ suggestions, onDismiss, rules, onToggleRule }) {
  return (
...
function Tasks({ tasks, dragTaskId, setDragTaskId, moveTask }) {
  return (
    <div>
      <div className="mb-4 flex items-center gap-2">
...
function Inbox({ emails, filter, setFilter, markEmail }) {
  const filters = [
    { key: "all", label: "All" },
